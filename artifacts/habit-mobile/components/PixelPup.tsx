...
function fillColor(ch: string, palette: PetPalette): string {
  switch (ch) {
    case "B":
      return palette.B;
    case "X":
...
export function PixelPup({ slug, size = 220 }: { slug: string; size?: number }) {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols && x < row.length; x++) {
      ...
      if (ch === ".") continue;
      rects.push(
        <Rect key={`${x}-${y}`} x={x} y={y} width={1} height={1} fill={fillColor(ch, palette)} />
      );
    }
  }
