...
4. **Production deploy (web)**: **Netlify only** (`netlify.toml`) + separate Node host for API (e.g. Render). Do **not** use Vercel for this project — root `vercel.json` sets `git.deploymentEnabled: false`. Set Netlify env vars (`VITE_CLERK_PUBLISHABLE_KEY`, etc.), keep `/api/*` redirects in `netlify.toml`, and configure Clerk allowed origins for the Netlify / custom domain.
